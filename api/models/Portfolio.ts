import { genrateSlug } from "@/lib/utils";
...
// index for better peorformance

// This creates FULL TEXT SEARCH on these fields.
...
// 👉 Gives latest portfolios first
PortfolioSchema.index({ createdAt: -1 });

// 👉 Recently edited portfolios first
PortfolioSchema.index({ updatedAt: -1 });

// 👉 Trending / popular portfolios.
PortfolioSchema.index({ viewCount: -1 });

// pre-save middleware to genrate slug if not provided
PortfolioSchema.pre("save", function () {
  if (!this.slug && this.profile?.name) {
    this.slug = genrateSlug(this.profile.name as string);
  }
});

// preSave middleware to ensure slug uniqueness
PortfolioSchema.pre("save", async function () {
  if (this.isModified("slug")) {
    const existingPortfolio = await (
      mongoose.models.Portfolio as PortfolioModel
    ).findOne({
      slug: this.slug,
      _id: { $ne: this._id },
    });

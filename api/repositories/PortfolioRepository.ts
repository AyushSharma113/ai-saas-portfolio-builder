...
import { CreatePortfolioRequest, PortfolioFilters, PortfolioResponse, UpdatePortfolioRequest } from "@/types/portfolio";
...
export class PortfolioRepositoryClass extends BaseRepository<PortfolioDocument> {
  ...
  async findBySlug(slug: string): Promise<PortfolioDocument | null> {
    try {
      ...
      throw new Error(
        `failed to load portfolio by the slug: ${
  ...
  async findByUserId(
    ...
    try {
      ...
      const portfolios = await Portfolio.aggregate([
        { $match: query }, // filters portfolio by your query
        {
          $lookup: {
            from: "templates",
        ...
        { $unwind: "$templateId" }, //flatten template array
        {
          $lookup: {
            ...
            as: "views",
          }, //fetches all view records of this portfolio.
        },
        {
          $addFields: {
  ...
  async findAll(filters: PortfolioFilters = {}): Promise<PortfolioResponse> {
    try {
      ...
      // Build query
      const query: QueryFilter<PortfolioDocument> = {
        status: "published",
      ...
      const portfolios = await Portfolio.find(query)
        ...
        .sort(sort)
        .skip(skip)
        .limit(limit); //Restricts number of documents returned
  ...
  async isSlugTaken(slug: string, excludeId?: string): Promise<boolean> {
    try {
      const query: QueryFilter<PortfolioDocument> = { slug };
...
// Singleton instance of PortfolioRepositoryClass for app-wide usage
